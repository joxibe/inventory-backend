...
public class CategoryServiceImpl implements ICategoryService{
	...
	public ResponseEntity<CategoryResponseRest> searchById(Long id) {
		...
		try {
			...
			if(category.isPresent()) {
				...
				response.setMetadata("Respuest nok","-1", "Categoria encontrada");
			}else {
				response.setMetadata("Respuest nok","-1", "Categoria no encontrada");
		...
		try {
			...
			if(categorySearch.isPresent()) {
				...
				if(categoryToUpdate != null) {
					...
					response.getCategoryResponse().setCategory(list);
					response.setMetadata("Respuesta Ok", "ok", "Categoria actualizada");
				} else {
					response.setMetadata("Respuest nok","-1", "Categoria no actualizada");
					return new ResponseEntity<CategoryResponseRest>(response, HttpStatus.BAD_REQUEST);
				}
			...
			response.setMetadata("Respuest nok","-1", "Error al actualizar la categoria");
			e.getStackTrace();
			return new ResponseEntity<CategoryResponseRest>(response, HttpStatus.INTERNAL_SERVER_ERROR);		
		}
		return new ResponseEntity<CategoryResponseRest>(response, HttpStatus.OK);
	...
	public ResponseEntity<CategoryResponseRest> deleteById(Long id) {
		...
		try {
			...
			response.setMetadata("Respuest nok","-1", "Error al eliminar");
			e.getStackTrace();
			return new ResponseEntity<CategoryResponseRest>(response, HttpStatus.INTERNAL_SERVER_ERROR);		
		}
		return new ResponseEntity<CategoryResponseRest>(response, HttpStatus.OK);
